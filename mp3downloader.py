...
class App(tk.Tk):
    ...
    def _ctrl_v(self, e=None):
        try:
            ...
            if "spotify.com" in c:
                self.sp_var.set(c)
                self._log("🟢 Spotify erkannt...", "accent")
    ...
    def _build_ui(self):
        ...
        self._th = self._sbc.create_rectangle(1, 2, 7, 50,
                   fill=ACCENT, outline="")
    ...
    def _smooth(self):
        ...
        diff = self._sy - cur
        if abs(diff) < 0.0004:
            self._cvs.yview_moveto(self._sy)
            self._sa = False
            return
    ...
    def _build_settings(self, p):
        ...
        mk_btn(hdr, "✕  Schließen", self._toggle_settings,
               bg=CARD, hover=RED, fg=TEXT2,
               font=("Segoe UI", 9, "bold"), px=12, py=6).pack(side="right")
        ...
        tk.Label(fr, textvariable=self.output_dir, font=("Segoe UI", 9),
    ...
    def _build_main(self, p):
        ...
        mk_btn(rgt, "⚙️", self._toggle_settings,
        ...
        # Progressbar – einfache ttk ohne custom style name
        self.prog = ttk.Progressbar(p, mode="indeterminate", length=200)
    ...
    def _sec(self, p, icon, title, sub):
    ...
    def _update_thread(self):
        ...
        try:
            ...
            self._log(f"🆕 v{nv} verfügbar!", "accent")
            if IS_EXE:
